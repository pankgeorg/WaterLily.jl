...
callable `D(I)` — all face-averaged like the momentum viscosity.
...
# Kept separate from `_νf` so the momentum path stays array-free while the
# scalar-transport building block can take a diffusivity field directly.
...
"""
    Flow{D::Int, T::Float, Sf<:AbstractArray{T,D}, Vf<:AbstractArray{T,D+1}, Tf<:AbstractArray{T,D+2}, Nf, Lf}
...
"""
struct Flow{D, T, Sf<:AbstractArray{T}, Vf<:AbstractArray{T}, Tf<:AbstractArray{T}, Nf, Lf} <: AbstractFlow{D,T}
    # Fluid fields
    ...
    function Flow(N::NTuple{D}, uBC; mem=Array, Δt=0.25, ν=0., g=nothing,
        ...
        new{D,T,typeof(p),typeof(u),typeof(μ₁),typeof(ν_store),typeof(λ)}(u,u⁰,fv,p,σ,V,μ₀,μ₁,uBC,T[Δt],ν_store,g,exitBC,perdir,λ)
    end
end
